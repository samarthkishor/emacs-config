...
** Font
   ...
   Source: https://github.com/jming422/fira-code-mode/blob/master/fira-code-mode.el
                    ...
                    (when s
                      (let* ((code (+ #Xe100 idx))
                        ...
                        (while (< n width)
                          ...
                          (setq n (1+ n)))
                        (cons s (append prefix suffix
                                        (list (decode-char 'ucs code)))))))
                  list))))
...
*** Autocomplete
    ...
    Company is a solid completion engine for Emacs.
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (with-eval-after-load 'ocamlformat
        (progn
          ...
          (let ((ocamlformat-path (concat (substring (shell-command-to-string "eval $(opam env) && echo $OPAM_SWITCH_PREFIX") 0 -1) "/bin/ocamlformat")))
            (if (executable-find ocamlformat-path)
                (progn
                  (setq ocamlformat-command "/Users/samarth/Projects/AdventOfCode_2019/_opam/bin/ocamlformat")
